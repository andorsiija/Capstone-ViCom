...
function renderUploadedArtists(artworks){
	...
	artistGrid.innerHTML=uploadedArtists.map((work)=>`<article class="artist-card" data-style="${artistStyle(work.category)}" data-name="${work.artist.toLowerCase()}" data-artist-id="${work.artistId}"><div class="artist-photo" style="background-image:url('${work.image}')"><a class="artist-photo-link" href="../Account%20functions/artist.html?id=${encodeURIComponent(work.artistId)}" aria-label="View ${work.artist} profile"></a><button class="save" aria-label="Save ${work.artist}">♡</button></div><div class="artist-info"><div><h3>${work.artist}</h3><p>${work.category} · ${work.detail||'Original artwork'}</p></div><strong>from ${work.price}</strong></div></article>`).join('');
